...
function HistoricalCharts() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        // Solo usar los últimos 15 registros (opcional)
        const ultimos = data.slice(-15);

        setLabels(
          ultimos.map((item: { fecha: string | number | Date }) =>
            new Date(item.fecha).toLocaleString()
          )
        );
        setTemperatura(ultimos.map((item: { temperatura: number }) => item.temperatura));
        setHumedad(ultimos.map((item: { humedad: number }) => item.humedad));
        setLluvia(ultimos.map((item: { lluvia: number }) => item.lluvia));
        setSol(ultimos.map((item: { intensidad_solar: number }) => item.intensidad_solar));
      })
      .catch((error) => {
  ...
  return (
    <div className={`main-layout ${isOpen ? '' : 'collapsed'}`}>
